...
<body class="bg-light">
    <div class="container py-5">
        ...
        <h1 class="text-success mb-4">Quản lý Salers</h1>
                ...
                <tbody>
                    <% for (Admin a : vec) { %>
                        <tr>
                            ...
                            <td><%= a.getFullName() %></td>
                            <td>
                                <a class="btn btn-outline-primary btn-sm" href="AdminController?target=Saler&service=Detail&ID=<%= a.getId() %>">View</a>
                            </td>
                            <td>
                                <a class="btn btn-sm <%= a.isActive() ? "btn-danger" : "btn-success" %>" 
                                   href="AdminController?target=Saler&service=Ban&ID=<%= a.getId() %>&status=<%= a.isActive() %>">
        ...
        <!-- Add button -->
        <div class="mt-3">
            <a href="AdminController?target=Saler&service=Add" class="btn btn-outline-success">+ Add New Saler</a>
        </div>
    </div>
